...
func TestFemaleToFemale(t *testing.T) {
	left, right, doneChan, errChan := connector.FemaleToFemale(context.Background(), "localhost:0", "localhost:0")
	...
	go func() {
		conn, err := net.Dial("tcp", left)
		if err != nil {
			t.Error(err)
			return
		}

		result, err := writeAndReadAll(conn, sendToLeft)
		if err != nil {
			t.Error(err)
			return
		}
	...
	go func() {
		conn, err := net.Dial("tcp", right)
		if err != nil {
			t.Error(err)
			return
		}

		result, err := writeAndReadAll(conn, sendToRight)
		if err != nil {
			t.Error(err)
			return
		}
...
func TestFemaleToFemaleCancellation(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 1 * time.Second)
	defer cancel()

	_, _, doneChan, _ := connector.FemaleToFemale(ctx, "localhost:0", "localhost:0")
